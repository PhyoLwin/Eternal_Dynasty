...
echo "<span style='color: rgb(46, 8, 8);;'>မွေးနေ့       :စနေ<br><br>";

for ($i = 0; $i < 4; $i++) {
    // Select random bfirst_name which is not null
    $sql1 = "SELECT `gfirst_name` FROM `sat_girl` WHERE `gfirst_name` IS NOT NULL ORDER BY RAND() LIMIT 1";
    $result1 = $conn->query($sql1);

    // Select random bmiddle_double
    $sql2 = "SELECT `gmiddle_double` FROM `sat_girl` WHERE `gmiddle_double` IS NOT NULL ORDER BY RAND() LIMIT 1";
    $result2 = $conn->query($sql2);

    // Select random blast_name
    $sql3 = "SELECT `glast_name` FROM `sat_girl` WHERE `glast_name` IS NOT NULL ORDER BY RAND() LIMIT 1";
...
for ($i = 0; $i < 2; $i++) {
    ...
    $sql2 = "SELECT `gfirst_double` FROM `sat_girl` WHERE `gfirst_double` IS NOT NULL ORDER BY RAND() LIMIT 1";
    $result2 = $conn->query($sql2);
...
for ($i = 0; $i < 1; $i++) {
    ...
    if (!empty($bfirst_name)) {
        $sql3 = "SELECT `glast_name` FROM `sat_girl` WHERE `glast_name` IS NOT NULL 
